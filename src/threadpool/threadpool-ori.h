...
public:
    ...
    explicit ThreadPool(size_t threadCount = 8): pool_(std::make_shared<Pool>()) {
            ...
            for(size_t i = 0; i < threadCount; i++) {
                ...
                std::thread([pool = pool_] {
                    ...
                    while(true) {
                        ...
                        if(!pool->tasks.empty()) {
                            ...
                            pool->tasks.pop();
                            locker.unlock();
                            // task的执行无所谓，不需要保证互斥性，可以与线程的构造并行执行
                            task();
                            locker.lock();
                        }
                ...
                }).detach();    // detach可以将线程分离，可以与主线程并行执行，主线程不需要等待，不然for循环就要等待while循环
            }
    }
    ...
    /**
     * @brief 该函数主要的作用是用来添加任务，扮演着生产者的角色；
     * @param task 要添加的任务，这里设定为右值，提高效率；
     */
    template<class F>
    void AddTask(F&& task) {
        {
...
private:
    /**
     * @brief 将Pool理解为一个池子，池子里有这些工具：互斥锁、同步变量、池子状态、任务队列；
     */
    struct Pool {
    ...
    std::shared_ptr<Pool> pool_;    // 指向池子的智能指针，因为会被多个线程使用，因此定义为shared_ptr，这里可以联系之前unique_ptr指针指向的日志队列理解
};
